...
**Revision note (2.1.0):** Corrects two errata in the §8 example payload and replaces the underspecified `consistency` term in §5.4 with an implementable definition. Because the confidence formula feeds `placement_mode`, this is a scoring change and takes a `schema_version` bump to `2.1.0` under the rule in §12.12. No field was added, removed, or renamed — a 2.0.0 consumer stays wire-compatible.
...
| C18 | **`consistency` term in confidence** *(2.1.0)* | "1 − (variance across domain scores that cross-checks would predict should move together)" | Explicit benchmark-item agreement rate, bounded 0–1, with a defined `n < 4` fallback (§5.4) | The v1 wording named no variance, no scale, and no pairing rule, and could return values outside 0–1. Not implementable. |
| C19 | **Example-payload errata** *(2.1.0)* | `compass_overall_score: 61`, `recommended_phase: 4` | `53` and `5` respectively, with the arithmetic shown (§8, E1/E2) | `61` was the unweighted mean of six non-speech domains; §5.3's weighted formula gives 53.2. Caught during implementation, not by any test — see the note in §5.3 on why. |
...
The weights sum to exactly 1.00, so `compass_overall_score` is on the same 0–100 scale as each domain score. Any change to the weight set must preserve that sum, or the overall score silently stops being comparable to its own inputs.

> **`compass_overall_score` currently drives nothing.** In v1.0.0 it was consumed by the §6.2 ELSE branch ("phase closest to overall_score midpoint"). v2.0.0 replaced that with nearest-distance on the candidate's primary driver domain (C7), which reads domain scores directly. The overall score is now display-and-analytics only: no placement decision, no red flag, no confidence term reads it.
>
> This is why erratum E2 (§8) survived two document versions undetected — nothing downstream would have broken. Treat that as a standing caution rather than a defect: an unconsumed number is not self-checking, so it needs a unit test asserting the formula against a fixed fixture, since no integration path will ever catch it drifting. If it stays unconsumed through validation (§12), the honest options are to give it a job or drop it from the payload — not to keep shipping a number no code trusts.
...
consistency  = agreement rate between the benchmark checklist and the domain scores (below)
confidence   = round((0.6 * completeness + 0.4 * consistency), 2)   → 0.00–1.00
```

**`consistency`, specified.** *(v1.0.0 and v2.0.0 read "1 − (variance across domain scores that benchmark-checklist cross-checks would predict should move together)", which named no variance, no scale, and no pairing rule, and could return values outside 0–1. It was not implementable and is replaced.)*

Each functional benchmark item *b* in §3.9 carries two design-time tags:

- `predicted_domain(b)` — the one domain that item is a behavioural proxy for. Example: "combines two words" → `expressive_language`; "asks for help from an adult who isn't a parent" → `functional_communication`.
- `threshold(b)` — the `adj_compass_domain_score` at or above which a "yes" on that item is the expected answer. A design-time constant per item, on the 0–100 scale.

For each answered benchmark item, compare the caregiver's yes/no against the domain score the rest of the survey produced:

```
score  = adj_compass_domain_score(predicted_domain(b))
τ      = threshold(b)

agreement(b) =
    1.0   if (answer == yes AND score >= τ) OR (answer == no AND score < τ)
    0.5   if disagreeing but |score - τ| <= 10        # near-miss band
    0.0   otherwise

consistency = sum(agreement(b)) / n_benchmark_answered
```

This is bounded to 0.0–1.0 by construction, needs no normalization constant, and uses the checklist for exactly the cross-check purpose §2.9 and §3.9 claim for it.

The ±10 near-miss band exists because a threshold is a design-time estimate, not a measured boundary. Without it a child scoring 44 against a τ of 45 is penalised identically to one scoring 4 — which would make `confidence` mostly a measure of how well the thresholds happen to be calibrated, rather than of whether the caregiver's answers hang together.

**Too few benchmark items.** With fewer than 4 answered, an agreement rate over 1–3 items is noise. In that case:

```
confidence = min(round(completeness, 2), 0.74)
```

The consistency term is omitted rather than defaulted, and the result is capped below the `start_directly` threshold. We could not cross-check the answers, so the instrument does not claim high confidence — capping is the honest failure mode, and it routes the child to a readiness module rather than blocking them.

**Directional asymmetry is deliberately not modelled.** "Yes on the item, low domain score" (possible over-reporting) and "no on the item, high domain score" (possible under-reporting) mean different things clinically, but both are equally strong evidence that the responses do not hang together — which is all `confidence` measures. Store the direction per item so §12's bias review can look at it later; do not weight it here.

*Provisional status: the per-item `threshold(b)` values, like `age_weight_factor` (§14.7), are clinical-team estimates that do not yet exist. Until they do, the recommendation is the same — ship them explicitly rather than implicitly. Until thresholds are set, force the `n < 4` path for every assessment so no child receives a `start_directly` placement backed by an uncalibrated consistency term.*
...
```json
{
  ...
  "schema_version": "string, e.g. '2.1.0'",
...
> **Two errata carried over from the v1.0.0 example, both corrected above. Check any test fixture copied from the old document.**
>
> **E1 — `recommended_phase`.** v1.0.0 returned `4` for `functional_communication: 44`. Under §6.1/§6.2 the band 40–55 maps to Phase 5, not Phase 4.
>
> **E2 — `compass_overall_score`.** v1.0.0 and v2.0.0 both showed `61`. The §5.3 weights (which sum to exactly 1.00) applied to these domain scores give **53.2 → 53**. The `61` was the unweighted mean of the six non-speech domains (364 ÷ 6 = 60.67), i.e. a different formula that excluded `speech_sound` entirely and dropped all weighting. §5.3 is authoritative; the example was wrong, not the formula. If anyone ever intends the unweighted-and-speech-excluded version instead, that is a substantive change to §5.3 requiring a `schema_version` bump — it must not be reintroduced by editing an example.
...
9. **`threshold(b)` values for the consistency cross-check are unset.** §5.4 now defines the mechanism, but the per-benchmark-item domain thresholds are clinical-team estimates that do not yet exist. Until they are set, every assessment takes the `n < 4` path and is capped at `confidence = 0.74` — meaning **no child receives a `start_directly` placement at launch**, and every one gets a readiness module. That is a deliberate, conservative fallback rather than a blocker, but it is a real product consequence and should be a known launch state, not a surprise.
10. **`compass_overall_score` is computed but consumed by nothing** (see the note in §5.3). Decide during validation whether to give it a role or drop it from the payload. Meanwhile it needs a unit test against a fixed fixture, because no integration path exercises it.
11. **Was the overall score ever meant to exclude `speech_sound`?** The `61` erratum was internally consistent with a formula that dropped `speech_sound` and all weighting. That is most likely just a hand-computed mistake in the original draft, and §5.3 has been treated as authoritative — but if there was ever an intent to keep a speech-independent composite (which would have some logic to it, since Phase 12 exists precisely for children whose speech is an isolated gap), that intent is now lost and worth reconstructing before validation rather than after.
